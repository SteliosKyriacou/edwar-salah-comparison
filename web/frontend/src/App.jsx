  ...
  return (
    <>
      ...
      <div className="contact-footer">
        ...
        <a href="mailto:stelios@saisho.ai" className="contact-email">stelios@saisho.ai</a>
      </div>
    </>
  )
}
...
function UsagePage() {
  ...
  const [formTotalLimit, setFormTotalLimit] = useState(-1)
  ...
  function handleEditKey(k) {
    ...
    setFormTotalLimit(k.total_limit ?? -1)
        ...
        <>
          ...
          <div className="usage-card" style={{ padding: 24, marginTop: 12 }}>
            ...
            <div style={{ background: '#070a13', border: '1px solid var(--border)', borderRadius: 6, padding: '14px 18px', maxHeight: '180px', overflowY: 'auto', fontFamily: 'monospace', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
              {filteredRuns.length > 0 ? (
                [...filteredRuns].reverse().slice(0, 10).map((run, index) => {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }} className="usage-grid">
            <div className="usage-card" style={{ marginBottom: 0 }}>
              ...
              <div className="usage-item">
                <span className="usage-label">Total Budget:</span>
                <span className={`usage-value ${info.total_limit < 0 ? 'unlimited' : ''}`}>
                  {info.total_limit < 0 ? 'Unlimited' : `${info.total_limit} evaluations`}
                </span>
              </div>
              <div className="usage-item">
                <span className="usage-label">Budget Remaining:</span>
                <span
                  className={`usage-value ${info.total_remaining === 'unlimited' ? 'unlimited' : ''}`}
                  style={info.total_remaining !== 'unlimited' && info.total_remaining === 0 ? { color: 'var(--accent-red)' } : undefined}
                >
                  {info.total_remaining === 'unlimited'
                    ? 'Unlimited'
                    : `${info.total_remaining} of ${info.total_limit} left`}
                </span>
              </div>
            ...
            <div className="usage-card" style={{ marginTop: 32 }}>
              ...
              <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 0, padding: 20, marginBottom: 24 }}>
                ...
                <form onSubmit={handleSaveKey} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  ...
                  <div className="form-group">
                    <label style={{ display: 'block', fontSize: '0.78rem', marginBottom: 4, color: 'var(--text-secondary)' }}>Total Evaluation Budget</label>
                    <input
                      type="number"
                      required
                      placeholder="Use -1 for unlimited"
                      ...
                      onChange={(e) => {
                        ...
                        setFormTotalLimit(Number.isNaN(v) ? -1 : v)
                      }}
                      style={{ width: '100%', padding: '8px 12px', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: 0, color: '#fff', fontSize: '0.85rem' }}
                    />
                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: 4 }}>
                      Lifetime cap. Ticks down by one per completed evaluation and never resets.
                    </div>
                  </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-muted)' }}>
                      ...
                      <th style={{ textAlign: 'center', padding: '10px 8px' }}>Budget</th>
                    ...
                    {info.all_keys && info.all_keys.map((k) => {
                      ...
                      return (
                        <tr key={k.key} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                          ...
                          <td style={{ padding: '10px 8px', textAlign: 'center', color: k.total_limit < 0 ? 'var(--accent-green)' : (k.total_remaining === 0 ? 'var(--accent-red)' : 'var(--text-primary)') }}>
                            {k.total_limit < 0 ? 'Unlimited' : `${k.total_remaining} / ${k.total_limit}`}
                          </td>
