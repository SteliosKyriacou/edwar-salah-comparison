...
OUTPUT_FILE = os.path.join(BASE, "SM_V20_EDWARD_ONLY_LEGACY.csv")
PROGRESS_FILE = os.path.join(BASE, "sm_v20_legacy_progress.jsonl")
...
def main():
    ...
    print(f"Starting V20 Edward-Only LEGACY Audit from index {start_index} (n={len(df)})...", flush=True)
    ...
    print(f"V20 Edward-Only LEGACY Audit Complete: {OUTPUT_FILE} ({len(final_df)} rows)", flush=True)
