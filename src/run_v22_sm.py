...
OUTPUT_FILE = os.path.join(BASE, "SM_V22_EDWARD_SALAH.csv")
PROGRESS_FILE = os.path.join(BASE, "sm_v22_progress.jsonl")
...
def main():
    ...
    print(f"Starting V22 Edward+Salah Coordinated SM Audit from index {start_index} (n={len(df)})...", flush=True)
    ...
    print(f"V22 Edward+Salah Audit Complete: {OUTPUT_FILE} ({len(final_df)} rows)", flush=True)
